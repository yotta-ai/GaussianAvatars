...
def main(model_path: str, blendshapes: List[Dict], output_video: str, fps: int = 25):
    """Processes blendshapes based on timestamps and creates an animation."""
    save_folder = Path(output_video).parent / "frames"
    save_folder.mkdir(parents=True, exist_ok=True)

    cfg = Config(
        point_path=Path(model_path),
    ...
    interpolated_blendshapes = interpolate_blendshapes(blendshapes, fps)

    # Render animation
    # List to track generated frames
    generated_frames = []

    # Render animation
    for i, frame_blendshapes in enumerate(interpolated_blendshapes):
        print(f"Rendering frame {i + 1}/{len(interpolated_blendshapes)}")
        print("Blendshapes: ", frame_blendshapes)
        viewer.apply_blendshapes(frame_blendshapes)

        cam = viewer.prepare_camera()
        render_output = render(cam, viewer.gaussians, cfg.pipeline, torch.tensor(cfg.background_color).cuda())
        ...
        # Track only generated frames
        generated_frames.append(frame_path)

    # Convert generated frames to video
    if not generated_frames:
        print("No frames were rendered! Exiting...")
        return

    first_frame = cv2.imread(str(generated_frames[0]))
    ...
    print(f"Video successfully saved at: {output_video}")


### ========================== RUN THE PROGRAM ========================== ###

if __name__ == "__main__":
    with open("audio2.json", "r") as file:
        blendshapes_data = json.load(file)
    print("Len of data: ", len(blendshapes_data))
    main("media/306/point_cloud.ply", blendshapes_data, "audio2.mp4", fps=30)
